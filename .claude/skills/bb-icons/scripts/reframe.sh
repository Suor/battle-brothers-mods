#!/usr/bin/env bash
# Re-frame an icon that was generated WITH a baked frame on a green background: strip that frame off
# and put the project's standard rim on instead, so it matches the rest. Keeps the approved art + its
# interior. Use this to RESCUE a good generation that only has the wrong frame/background — much
# better than regenerating from scratch and getting something wildly different.
#
#   key green -> trim -> crop to the inner circle (drops the baked frame) -> fit to the disc area
#   -> sharpen -> overlay the rim.
#
# Usage: reframe.sh <framed_art.png> <out_base> <rim.png>
#   rim.png = a transparent-centre ring PNG (e.g. the AC perk blank works as a rim source).
#   env: INNER = inner-crop radius as fraction of the framed circle (default 0.82),
#        DISC  = fitted disc diameter px (default 50), USM/SHP/SAT/CON = sharpen/colour knobs
set -euo pipefail
IN="${1:?framed art}"; OUT="${2:?out base}"; RIM="${3:?rim png}"
SIZE=56; DISC="${DISC:-50}"; FUZZ=22%; INNER="${INNER:-0.82}"
...
# mask to the inner circle (drop the baked frame), then TRIM to that circle's bbox so it fills the
# disc area (otherwise the subject shrinks and leaves a gap to the rim).
...
# Force 8-bit RGBA: the game engine (WebCore) can't read 16-bit PNGs, which IM would otherwise inherit.
