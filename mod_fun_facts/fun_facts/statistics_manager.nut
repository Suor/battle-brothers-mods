::mods_hookNewObjectOnce("statistics/statistics_manager", function (o)
{
	...
	o.onSerialize = function( _out ) {
		...
		::FunFacts.Mod.Serialization.flagSerialize("FallenWithStats", FunFacts, this.m.Flags);
		onSerialize(_out);
	}
	...
	o.onDeserialize = function( _in ) {
		...
		if (::FunFacts.Mod.Serialization.isSavedVersionAtLeast("0.1.0", _in.getMetaData())) {
			local FunFacts = ::FunFacts.Mod.Serialization.flagDeserialize("FallenWithStats", [], null, this.m.Flags);

			foreach (i in FunFacts) {
				this.m.Fallen[i].FunFacts <- ::new("scripts/mods/fun_facts/fun_facts");
				this.m.Fallen[i].FunFacts.onDeserialize(
